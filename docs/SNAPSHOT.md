...
- **수익화 착수(도서 어필리에이트)**: 결과 본문 아래 "Top pick + 추천 도서" 블록(`BookModule` · 표지 `coverImage` 폴백=제목 placeholder). 유형에 `books` 있을 때만 노출(현재 **다윗만 시드**). `AMAZON_TAG` 상수 비어있음 → Amazon Associates 태그 발급 후 입력. 광고는 Phase 2.
...
1. 도메인 초록불 확인(Vercel Refresh) → **Amazon Associates 가입**(라이브 도메인 제출) → 태그 발급 → `AMAZON_TAG` 입력 + **16유형 추천 도서 채우기**(큐레이션 완료 — 유형별 2권 목록 확보)
