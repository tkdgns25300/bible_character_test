...
- 단계: **Phase 1 — 콘텐츠·디자인·Vercel 배포·도메인·수익화(도서 어필리에이트) 라이브.** Amazon 가입·세금(W-8BEN 0%)·태그·16유형 도서(실표지·`/dp` 직접링크) 완료. **도서 블록 위치/디자인은 다듬는 중(미확정).** 분석·이메일 미연동.
...
- **수익화(도서 어필리에이트) 라이브**: 유형당 **단일 베스트셀러 1권**(실표지 `public/images/books/*` + **`/dp/ASIN` 직접 링크**, 16권 전부 검증). 노출 = **매치 카드 밑 'Read next' 컴팩트 스트립**(`BookStrip`, 어필리에이트 고지 포함) + **스크롤 시 하단 슬림 바**(`BookStickyBar` — 인라인 카드가 화면 밖이면 등장·공유 섹션에선 숨김·닫기 가능). 태그 `biblecharacte-20`, 세금 W-8BEN 검증(원천징수 0%). 광고는 Phase 2.
- ⏳ **도서 선정은 확정**(유형당 대중 베스트셀러 1권·실표지·직접링크). **블록 위치/디자인은 진행 중**: 현재 시안2(매치 밑 스트립)+슬림 바 적용했으나 **계속 다듬는 중(미확정)**.
- **배포**: GitHub→**Vercel 자동배포** 연결(push=production 배포, MCP로 모니터). 도메인 **`biblecharactertest.life`**(hosting.kr) 연결 중 — DNS(A `@`→`216.198.79.1`, CNAME `www`→vercel) 등록, **non-www 대표**. `NEXT_PUBLIC_SITE_URL`이 `SITE_URL`·표시 host(`SITE_HOST`) 구동.
...
1. ⏳ **도서 블록 위치/디자인 마무리** — 시안2(매치 밑 'Read next' 스트립)+슬림 바 적용, **계속 조정 중**(아직 미확정).
2. (선택) 분석(Plausible/GA4)·Search Console / 이메일 수집(현재 보류)
3. `/types` 디렉토리 보강, **Footer를 `/test`에서 숨김**(라우트 그룹)
4. **`/test`(질문 플로) 디자인 검토**
5. (선택) 공유 이미지 표지 실제 이미지 / 스토리 9:16, favicon `.ico`

## 열린 질문

- 이메일 수집 도입 시점(현재 보류) / 자체 PDF 상품 제작 여부
- **디자인** — 결과 페이지(히어로·궁합·In Scripture·Newsreader·공유) 완료, **도서 블록 위치/디자인 조정 중**. **`/test`(질문 플로) 검토 대기**
