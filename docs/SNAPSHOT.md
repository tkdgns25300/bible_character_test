...
- 단계: **Phase 1 — 콘텐츠·디자인 완료 + Vercel 배포·도메인 연결 진행 + 수익화(도서 블록) 착수.** Amazon Associates 가입·실연동(이메일/분석)이 남음.
...
- **수익화(도서 어필리에이트) 활성**: 결과 본문 아래 "Top pick + 추천 도서" 블록(`BookModule` · 표지 `coverImage` 폴백=제목 placeholder). **Amazon Associates 태그 `biblecharacte-20` 발급·입력 완료, 16유형 도서 전부 입력 → 어필리에이트 링크 라이브**(클릭→구매 시 수수료). 광고는 Phase 2.
- ⚠️ **추후 재작업(placeholder)**: **도서 선정 + 도서 블록 디자인은 임시.** 현재 들어간 책 목록·노출 UI는 잠정이며, 큐레이션·디자인을 추후 새로 결정한다(이번엔 태그 활성·발행만이 목적). 세금정보(W-8BEN)는 Amazon에서 `Incomplete` → 완료 필요.
...
1. **Amazon 세금정보(W-8BEN) 완료** — 계정에서 `Incomplete` 상태. 미완 시 수익 30% 원천징수.
2. ⚠️ **(추후) 도서 선정 + 도서 블록 디자인 전면 재작업** — 현재 16유형 도서·노출 UI는 임시(placeholder).
3. (선택) 분석(Plausible/GA4)·Search Console / 이메일 수집(현재 보류)
...
- **디자인** — 결과 페이지(히어로·궁합·In Scripture·Newsreader·공유·도서 블록) 완료. **`/test`(질문 플로) 검토 대기**
